    $(document).ready(function(e) {
        ...
        FormData.prototype.getPriceEos = function(argument){
            ...
            return (preview*data.month).toFixed(3); 
        };
        ...
        FormData.prototype.createViewEos = function(){
            ...
            view.find('.edit_buy_message').text(data.month + '个月');
            return view;
        };
        ...
        FormData.prototype.renderFormEos = function(){
            ...
            var progress = $('#progressEos').progress({
                totalStep: 14,
                format:function(step){
                    if(step<12){
                        return step;
                ...
                onChange: function(){
                    $('#progressTextEos').val(progress.getValue());
                    that.attrEos('month',progress.getValue());
                }
            });
            progress.setValue(that.attrEos('month'));

            var progressDom = $('.progressEos'),
                progressInput = price.getProgressInput();
            progressDom.find('#progressTextEos').remove();

            $.each(progressInput,function(index, element) {
                var input = $('<input class="time" type="text" id="progressTextEos" />');
                input.attr('value',data.month);
                input.blur(function () {
                    progress.setValue(parseInt($('#progressTextEos').val()));
                });
                progressDom.append(input);
            });
